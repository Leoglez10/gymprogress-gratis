...
export const HomeView: React.FC<Props> = ({ onStartLogging, onGoToExercises, onGoToProfile, onGoToHelp, userProfile }) => {
  ...
  const handleDeleteSession = async (sessionId: string) => {
    ...
    try {
      // Try to delete from Supabase if it's a UUID
      const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(sessionId);
      if (isUUID) {
        const result = await sessionService.deleteSession(sessionId, userProfile.id);
        if (!result.success) {
          alert(result.error || 'No se pudo eliminar la sesión en el servidor');
          setDeletingId(null);
          return;
        }
      }
